...
If `PROACTIVE` is `false`: do NOT proactively suggest other jstack skills during this session.
Only run skills the user explicitly invokes. This preference persists across sessions via
`jstack-config`.

If `PROACTIVE` is `true` (default): suggest adjacent jstack skills when relevant to the
