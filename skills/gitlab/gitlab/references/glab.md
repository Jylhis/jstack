...
For full details and GraphQL alternatives, see [glab/work-items.md](glab/work-items.md).
...
For issue links (`blocked_by`, `relates_to`), see [glab/issue-links.md](glab/issue-links.md).
For epic CRUD and comments, see [glab/epics.md](glab/epics.md).
...
`glab api --field position[new_line]=N` silently falls back to a non-inline comment when GitLab rejects the position. Always use JSON body via REST API instead -- see `../scripts/post-inline-comment.py` or [glab/inline-comments.md](glab/inline-comments.md).
