---
title: "jstack"
description: "Personal Claude Code configuration installed into ~/.claude via symlinks."
---

jstack is a single git repository that owns every file under `~/.claude/`. It
holds settings, the global `CLAUDE.md`, personal skills/agents/commands/hooks,
a runtime built with Nix, and a set of plugin bundles.

## What's in the box

| Layer | Path | Purpose |
|---|---|---|
| Settings | `settings.json` | Source of truth for `~/.claude/settings.json` |
| Global instructions | `CLAUDE.md` | Source of truth for `~/.claude/CLAUDE.md` |
| Personal skills | `skills/` | Dir-per-skill, `SKILL.md` inside each |
| Personal agents | `agents/` | Subagent `.md` files |
| Slash commands | `commands/` | Personal slash command `.md` files |
| Hooks | `hooks/` | Hook scripts referenced from `settings.json` |
| Plugins | `plugins/<name>/` | One directory per plugin bundle |
| Runtime | `runtime/default.nix` | `pkgs.buildEnv` with MCP servers and LSPs |
| Scripts | `scripts/install.bash` | Install entry point |

## Plugin bundles

Six plugin bundles ship in this repo. Each lives at `plugins/<name>/` with
its own `.claude-plugin/plugin.json`, optional `.mcp.json`, and a `skills/`
directory.

| Plugin | Skills | Notes |
|---|---|---|
| [rust-dev](/plugins/rust-dev) | 29 | Ownership, lifetimes, error handling, concurrency, domain packs, LSP analyzers |
| [golang-dev](/plugins/golang-dev) | 36 | Idioms, perf, testing, security, observability, gRPC, samber libraries |
| [nix-dev](/plugins/nix-dev) | 7 | Nix language, NixOS modules, flakes, devenv, home-manager, plus `mcp-nixos` MCP server |
| [productivity](/plugins/productivity) | 1 | Weekly session log appender |
| [skill-creator](/plugins/skill-creator) | 1 | Anthropic's official skill authoring + eval framework |
| [obsidian](/plugins/obsidian) | 5 | Obsidian markdown, canvas, bases, CLI, web extraction (kepano) |

## Next

<CardGroup cols={2}>
  <Card title="Install" icon="download" href="/install">
    Run `scripts/install.bash` to link everything into `~/.claude/`.
  </Card>
  <Card title="Develop" icon="code" href="/develop">
    Use the devenv shell, lint, and run repository checks.
  </Card>
  <Card title="Architecture" icon="diagram-project" href="/architecture">
    How the symlink layout, runtime, and plugins fit together.
  </Card>
  <Card title="Plugins" icon="puzzle-piece" href="/plugins/rust-dev">
    Browse the bundled plugins and what they ship.
  </Card>
</CardGroup>
