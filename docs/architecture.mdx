...
5. Builds the runtime via `nix-build runtime/default.nix` and links the
   result under `~/.local/state/jstack/runtime`.
6. Updates the shell `rc` so `~/.local/state/jstack/runtime/bin` is
   on `PATH`.

Anything the installer would displace is moved into a timestamped backup
under `~/.claude/.jstack-backups/<timestamp>/`. A `RESTORE.md` is
dropped alongside.
