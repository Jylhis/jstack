...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains contributor mode check', () => {
    ...
    expect(content).toContain('jstack_contributor');
...
describe('Codex generation (--host codex)', () => {
  ...
  test('codexSkillName mapping: root is jstack, others are jstack-{dir}', () => {
    // Root → jstack
  ...
  test('sidecar paths point to .agents/skills/jstack/review/ (not jstack-review/)', () => {
    ...
    expect(content).toContain('.agents/skills/jstack/review/design-checklist.md');
    ...
    expect(content).not.toContain('.agents/skills/jstack-review/design-checklist.md');
  });
...
describe('setup script validation', () => {
  ...
  test('link_claude_skill_dirs creates relative symlinks', () => {
    // Claude links should be relative: ln -snf "jstack/skill_name"
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('ln -snf "jstack/$skill_name"');
  });
