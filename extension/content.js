...
function renderRefPanel(refs) {
  ...
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    ...
    row.innerHTML = `<span class="jstack-ref-panel-id">${ref.ref}</span> <span class="jstack-ref-panel-role">${ref.role}</span> <span class="jstack-ref-panel-name">"${ref.name}"</span>`;
