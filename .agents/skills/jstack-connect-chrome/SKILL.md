...
_CONTRIB=$($JSTACK_BIN/jstack-config get jstack_contributor 2>/dev/null || true)
...
for _PF in $(find ~/.jstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do [ -f "$_PF" ] && $JSTACK_BIN/jstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
```
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.agents/skills/jstack/browse/dist/browse" ] && B="$_ROOT/.agents/skills/jstack/browse/dist/browse"
[ -z "$B" ] && B=$JSTACK_BROWSE/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.agents/skills/jstack/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.agents/skills/jstack/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.agents/skills/jstack/extension/manifest.json" ] && _EXT_PATH="$HOME/.agents/skills/jstack/extension"
