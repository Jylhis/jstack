...
Unless the user requests otherwise, use `claude-opus-4-6` with adaptive thinking (`thinking: {type: "adaptive"}`). Default to streaming for long input/output or high `max_tokens` to prevent timeouts. Use `.get_final_message()` / `.finalMessage()` to get the complete response when not handling individual stream events.
